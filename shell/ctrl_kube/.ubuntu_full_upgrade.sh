...
CRON_FULL_SCRIPT="$CRON_SUDO_COMMAND $CRON_SUDO_JOB > $CRON_LOG_STDOUT 2> $CRON_LOG_STDERR"
...
# sudo touch /vagrant/ubuntu_full_upgrade.log
# sudo chown vagrant:vagrant /vagrant/ubuntu_full_upgrade.log
# sudo touch $CRON_LOG_STDOUT $CRON_LOG_STDERR
# sudo chown vagrant:vagrant $CRON_LOG_STDOUT $CRON_LOG_STDERR

sudo apt-get update -yqq && sudo apt-get -yq full-upgrade > /vagrant/ubuntu_full_upgrade.log 2>&1
...
# add cronjob
sudo /bin/bash -c "cat <(crontab -l) <(echo \"$CRON_FULL_SCRIPT\") | crontab -"
